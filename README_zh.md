...
| **跑预置 alpha zoo 横评** | 501 个 alpha 因子（Qlib 158 + Kakushadze 101 + GTJA 191 + academic + PIT-safe fundamental + crypto），一行 CLI 在你选的 universe 上算 IC + IR + alive/reversed/dead 分类 |
...
<details>
<summary><b>Alpha Zoo</b> <sub>501 个预置 alpha，覆盖 7 个家族</sub></summary>

- 🧬 501 个横截面 alpha，算子层即禁用 lookahead
...
| **crypto** | 9 | 永续合约衍生品信号（资金费率、OI 变化、OI-价格背离、成交量比、振幅）+ 链上估值与资金流（NVT、MVRV z-score、活跃地址、交易所净流入） | 公开行情与链上数据 |
...
vibe-trading alpha list    # 浏览 501 个预置 alpha；支持 show / bench / compare / export-manifest 子命令
...
│   │   │
│   │   ├── factors/                # Alpha Zoo —— 7 个家族共 501 个 alpha
...
| **Alpha Zoo** | 501 个预置 alpha 因子（Qlib 158 + Kakushadze 101 + GTJA 191 + academic + fundamental + crypto），一行 CLI 跑横评，agent 集成，Web UI 浏览 | **已发布 0.1.8**，延续至 0.1.12 |
