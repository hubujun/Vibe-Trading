...
| **사전 빌드된 alpha zoo 벤치** | 480개의 alpha 인자(Qlib 158 + Kakushadze 101 + GTJA 191 + academic + PIT-safe fundamental + crypto)에 대해  한 줄 CLI로 IC + IR + alive/reversed/dead 분류 수행 |
...
<details>
<summary><b>Alpha Zoo</b> <sub>7개 패밀리에 걸친 480개 사전 빌 드된 quant alpha</sub></summary>

- 🧬 operator 계층에서 lookahead가 금지된 480개 cross-sectional alpha
...
| **crypto** | 9 | 무기한 선물 파생 신호(funding rate, OI change, OI-price divergence, volume ratio, amplitude) + 온체인 밸류에이션·플로우(NVT, MVRV z-score, active addresses, exchange netflow) | 공개 시장 및 온체인 데이터 |
...
vibe-trading alpha list    # 사전 빌드된 480개 alpha 탐색; show / bench / compare / export-manifest 서브커맨드 사용 가능
...
│   │   │
│   │   ├── factors/                # Alpha Zoo — 7개 패밀리에 걸친 480개 alpha
...
| **Alpha Zoo** | 7개 패밀리에 걸친 480개의 사전 빌드된 alpha 인자(Qlib 158 + Kakushadze 101 + GTJA 191 + academic + fundamental + crypto), 한 줄 CLI 벤치, agent 통합, Web UI | **0.1.8 출시 완료**, 0.1.12까지 확장 |
